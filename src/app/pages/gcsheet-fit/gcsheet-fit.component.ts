...
import { GcsheetFitService, FitRecord } from '../../services/gcsheet-fit.service';
import { ToastService } from '../../shared/toast/toast.service';
import { PaginationComponent } from '../../shared/pagination/pagination.component';

@Component({
  selector: 'app-gcsheet-fit',
  standalone: true,
          ...
          <ul class="navbar-nav me-auto">
            ...
            <li class="nav-item dropdown">
              ...
              <ul class="dropdown-menu" [class.show]="showGcsheetMenu">
                ...
                <li><a class="dropdown-item active" style="color: #0d6efd; font-weight: 500;"><i class="bi bi-rulers me-2"></i>Fit</a></li>
    ...
    <div class="container mt-5">
      <div class="card">
        <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
          <h4 class="mb-0"><i class="bi bi-rulers me-2"></i>Gcsheet Fit Master</h4>
          <div class="d-flex gap-2">
            ...
            <button class="btn btn-light btn-sm" (click)="openAddModal()">
              <i class="bi bi-plus-circle me-1"></i>Add Fit
            </button>
          </div>
        </div>
        ...
        <div class="card-body">
          <div class="mb-3">
            <div class="input-group">
              ...
              <input type="text" class="form-control" placeholder="Search by fit name..." [ngModel]="searchTerm()" (ngModelChange)="onSearchChange($event)">
            </div>
          </div>
          ...
          @if (loading()) {
            <div class="text-center py-4">
              ...
              <p class="mt-2">Loading fit records...</p>
            </div>
          } @else {
              ...
              <table class="table table-hover">
                <thead class="table-light">
                  <tr>
                    ...
                    <th class="sortable-header" (click)="toggleSort('pcsperpkt')">
                      Pcs Per Pkt
                      @if (sortField() === 'pcsperpkt') {
                        <i class="bi" [class.bi-caret-up-fill]="sortDirection() === 'asc'" [class.bi-caret-down-fill]="sortDirection() === 'desc'"></i>
                      }
                    </th>
                    <th>Actions</th>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title">{{ isEditMode() ? 'Edit' : 'Add' }} Fit</h5>
            ...
            <div class="modal-body">
              <form [formGroup]="form">
                ...
                <div class="mb-3">
                  <label class="form-label">Pcs Per Pkt</label>
                  <input type="number" class="form-control" formControlName="pcsperpkt" min="0">
                </div>
              </form>
            </div>
      ...
      <div class="modal show d-block" tabindex="-1" style="background-color: rgba(0,0,0,0.5);">
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <div class="modal-body"><p>Are you sure you want to delete this fit?</p></div>
            <div class="modal-footer">
  ...
  `,
  styles: [`.navbar .dropdown-menu { position: absolute; } .dropdown-item.active { background-color: #f8f9fa; font-weight: 500; } .sortable-header { cursor: pointer; } .sortable-header:hover { background-color: #e9ecef; }`]
})
export class GcsheetFitComponent implements OnDestroy {
  private service = inject(GcsheetFitService);
  ...
  openEditModal(item: any) { this.isEditMode.set(true); this.currentItem.set(item); this.duplicateError.set(false); this.form.patchValue({ name: item['name'], pcsperpkt: item['pcsperpkt'] || null }); this.showModal.set(true); }
  ...
  async saveData() {
    ...
    try {
      const data: { name: string; pcsperpkt?: number } = {
        name: formData.name!
      };
      if (formData.pcsperpkt) {
        data.pcsperpkt = parseInt(formData.pcsperpkt as any);
      }
      let result = this.isEditMode() ? await this.service.update(this.currentItem().id, data) : await this.service.create(data);
      if (result.success) { this.toastService.success(`Fit ${this.isEditMode() ? 'updated' : 'created'} successfully`); this.closeModal(); await this.loadData(); }
      else { this.toastService.error(result.error); }
  ...
  async confirmDelete() {
    ...
    this.saving.set(true);
    try {
      if (await this.service.delete(item.id)) { this.toastService.success('Fit deleted successfully'); this.closeDeleteModal(); await this.loadData(); }
      else { this.toastService.error('Failed to delete'); }
    } catch (err: any) { this.toastService.error(err.message); } finally { this.saving.set(false); }
  }

  exportToExcel(): void {
    this.isImporting.set(true); this.importProgress.set(0);
    setTimeout(() => {
      const ws = XLSX.utils.json_to_sheet(this.data().map(item => ({ 
        'Name': item['name'],
        'Pcs Per Pkt': item['pcsperpkt'] || ''
      })));
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'Fit Master');
      XLSX.writeFile(wb, `gcsheet_fit_export_${new Date().toISOString().split('T')[0]}.xlsx`);
      this.importProgress.set(100);
      setTimeout(() => { this.isImporting.set(false); this.importProgress.set(0); }, 500);
    }, 100);
  }
  ...
  async importFromExcel(event: Event): Promise<void> {
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (jsonData.length === 0) { this.toastService.error('No data found in the file'); return; }
        
        const newRecords: { name: string; pcsperpkt?: number }[] = [];
        for (const row of jsonData) {
          const name = row['Name'] || row['name'];
          const pcsperpkt = row['Pcs Per Pkt'] || row['pcsperpkt'];
          if (name) {
            const record: { name: string; pcsperpkt?: number } = { name: String(name).trim() };
            if (pcsperpkt) record.pcsperpkt = parseInt(pcsperpkt);
        ...
        await this.loadData();
        this.toastService.success(result.imported > 0 ? `Successfully imported ${result.imported} record(s)${result.failed > 0 ? `. ${result.failed} failed.` : '.'}` : `Import completed. ${result.failed} record(s) failed.`);
      } catch (err: any) { this.toastService.error(err.message || 'Failed to import file'); }
