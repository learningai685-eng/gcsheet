...
export class GcsheetPktmasterComponent implements OnInit {
  ...
  totalItems = computed(() => this.filteredPacketsWithoutPaging().length);
  ...
  async loadPackets() {
    ...
    try {
      const records = await this.pktmasterService.getAll();
      this.packets.set(records);
    } catch (err: any) {
    ...
    reader.onload = async (e) => {
      try {
        ...
        const newRecords = jsonData
          .map(row => ({
            pktno: row['Pkt No'] || row['pktno'] || null,
            pktname: row['Packet Name'] || row['pktname'],
        ...
        const result = await this.pktmasterService.batchCreate(newRecords, (done, total) => {
