...
import { GcsheetSaleinvService, SalesDetail } from '../../services/gcsheet-saleinv.service';
import { ToastService } from '../../shared/toast/toast.service';
import { AppGlobalsService } from '../../services/app-globals.service';
import { PaginationComponent } from '../../shared/pagination/pagination.component';

@Component({
  selector: 'app-gcsheet-saleinv',
  standalone: true,
                  ...
                  <tbody>
                    @for (inv of paginatedInvoices(); track inv.id) {
                      <tr>
                        ...
                        <td class="text-end">{{ inv['billamt'] | number:'1.2-2' }}</td>
                        <td class="text-end">{{ inv['nettamount'] | number:'1.2-2' }}</td>
                        <td>
                          <button class="btn btn-sm btn-outline-success me-1" (click)="printInvoiceDirect(inv)" title="Print Invoice">
                ...
                <div class="col-md-5">
                  <div class="form-floating">
                    <input type="text" class="form-control" formControlName="customername" id="customername" placeholder=" "
                           (keydown.enter)="focusTruckno($event)">
                ...
                <div class="col-md-2">
                  <div class="form-floating">
                    <input type="text" class="form-control" formControlName="transportation" id="transportation" placeholder=" "
                           (keydown.enter)="focusProductSearchFromHeader($event)">
                ...
                <div class="col-md-3">
                  <div class="form-floating">
                    <input type="text" class="form-control" formControlName="narration" id="narration" placeholder=" "
                           (keydown.enter)="focusFieldById('wgtslipno', $event)">
                    <label for="narration">Narration</label>
                  </div>
                </div>
                <div class="col-md-2 ms-auto">
                  <div class="form-floating">
                    <input type="text" class="form-control" formControlName="wgtslipno" id="wgtslipno" placeholder=" "
                           (keydown.enter)="focusFieldById('loadslipno', $event)">
                    <label for="wgtslipno">Wgt Slip No</label>
                  </div>
                </div>
                <div class="col-md-2">
                  <div class="form-floating">
                    <input type="text" class="form-control" formControlName="loadslipno" id="loadslipno" placeholder=" "
                           (keydown.enter)="focusFieldById('other1Desp', $event)">
                    <label for="loadslipno">Load Slip No</label>
                  </div>
                </div>
              </div>
                  ...
                  <table class="table table-sm table-bordered">
                    <thead class="table-light">
                      <tr>
                        ...
                        <th style="width: 300px">Packet Name</th>
                    ...
                    <tbody>
                      @for (detail of detailsArray.controls; track detail; let i = $index) {
                        <tr [formGroupName]="i">
                          <td>{{ i + 1 }}</td>
                          <td>
                            <input type="text" class="form-control form-control-sm" formControlName="particulars" placeholder="Particulars">
                          </td>
                          <td>
...
export class GcsheetSaleinvComponent implements OnInit {
  ...
  @ViewChild('productDropdown') productDropdown!: ElementRef<HTMLUListElement>;

  invoices = signal<any[]>([]);
  pktmasterList = signal<any[]>([]);
  mstfitList = signal<any[]>([]);
  ...
  masterDataLoading = signal(false);
  ...
  onInvoicePageChange(page: number): void {
    ...
    // Load data from server if not searching (when searching, we use client-side pagination)
    if (!this.searchTerm()) {
      this.loadMoreInvoices(page);
    }
  }
  ...
  paginatedInvoices = computed(() => {
    ...
    // When not filtering, invoices are already paginated from server
  ...
  mstfitMap = computed(() => {
    const map = new Map<string, any>();
    this.mstfitList().forEach(fit => {
      if (fit?.id) map.set(fit.id, fit);
    });
    return map;
  });
  ...
  netAmount = computed(() => {
    return this.billAmount() + this.toNumber(this.other1Amt()) + this.toNumber(this.other2Amt()) + this.toNumber(this.loadAmt()) + this.toNumber(this.gstAmt());
  });

  ngOnInit() {
    this.loadData();
    // Load master data in background without blocking UI
    this.loadMasterDataInBackground();
  }
  ...
  async loadData() {
    ...
    try {
      // Load only first page of invoices for fast initial display
      const result = await this.service.getSalesHeaderPage(1, this.invoicePageSize());
      this.invoices.set(result.items);
      this.totalInvoices.set(result.total);
    } catch (err: any) {
      this.toastService.error(err.message || 'Failed to load invoices');
    } finally {
      this.loading.set(false);
    }
  }

  private async loadMasterDataInBackground() {
    this.masterDataLoading.set(true);
    try {
      const [pktmaster, mstfit] = await Promise.all([
        this.service.getPktmasterAll(),
        this.service.getMstfitAll()
      ]);
      this.pktmasterList.set(pktmaster);
      this.mstfitList.set(mstfit);
    } catch (err: any) {
  ...
  async loadMoreInvoices(page: number) {
    try {
      const result = await this.service.getSalesHeaderPage(page, this.invoicePageSize());
      this.invoices.set(result.items);
      this.totalInvoices.set(result.total);
    } catch (err: any) {
      this.toastService.error(err.message || 'Failed to load invoices');
  ...
  onPktnameChange(index: number, event: Event) {
    const value = (event.target as HTMLInputElement).value;
    this.detailsArray.at(index).patchValue({ pktname: value });
  }
  ...
  moveProductSelection(step: number, event: Event) {
    ...
    setTimeout(() => {
      const dropdown = this.productDropdown?.nativeElement;
      if (dropdown) {
        const items = dropdown.querySelectorAll('.list-group-item');
        const highlightedItem = items[clamped] as HTMLElement;
        if (highlightedItem) {
  ...
  calculateAmount(index: number) {
    ...
    const weight = parseFloat(this.detailsArray.at(index).get('weight')?.value) || 0;
    const rate = parseFloat(this.detailsArray.at(index).get('rate')?.value) || 0;
    
    let amount = 0;
    if (qty !== 0 && weight !== 0) {
      amount = weight * rate;
    } else if (qty === 0 && weight !== 0) {
      amount = weight * rate;
    } else if (qty !== 0 && weight === 0) {
      amount = qty * rate;
    }
  ...
  async openEditModal(inv: any) {
    ...
    this.headerForm.patchValue({
      ...
      wgtslipno: inv['wgtslipno'] || '',
      loadslipno: inv['loadslipno'] || '',
    ...
    const details = await this.service.getSalesDetailByBillno(inv.id);
    
    if (details.length === 0) {
      this.addDetail();
    } else {
      // Build pktmaster map once for lookup
      const pktMap = new Map<string, any>();
      this.pktmasterList().forEach(p => p?.id && pktMap.set(p.id, p));
      
      // Create all form groups at once
      const newControls = details.map(det => {
        const pkt = pktMap.get(det['pktno']);
        return this.fb.group({
          id: [det['id'] || ''],
          pktno: [det['pktno'] || ''],
          pktname: [pkt ? pkt['pktname'] : ''],
          particulars: [det['particulars'] || pkt?.['pktname'] || ''],
          pktno_invalid: [false],
          qty: [det['qty'] || 0],
          weight: [det['weight'] || 0],
          rate: [det['rate'] || 0],
      ...
      // Replace all controls at once
      this.detailsArray.clear();
      newControls.forEach(ctrl => this.detailsArray.push(ctrl));
    }
  ...
  async saveInvoice() {
    ...
    await this.service.preAuthenticate();
    ...
    try {
      const headerPayload = {
        ...
        wgtslipno: headerData['wgtslipno'],
        loadslipno: headerData['loadslipno'],
      ...
      const deletePromise = this.isEditMode() 
        ? this.service.deleteSalesDetailsByBillno(headerId) 
      ...
      this.cancelForm();
      this.loadData(); // fire and forget - don't block UI
      this.saving.set(false);
    } catch (err: any) {
  ...
  async saveAndPrint() {
    ...
    try {
      // Pre-authenticate once before all operations
      await this.service.preAuthenticate();
      ...
      if (this.isEditMode()) {
        await this.service.updateSalesHeader(this.currentInvoice().id, headerPayload);
        headerId = this.currentInvoice().id;
        await this.service.deleteSalesDetailsByBillno(headerId);
      } else {
        const headerResult = await this.service.createSalesHeader(headerPayload);
      ...
      if (detailsToSave.length > 0) {
        await this.service.createSalesDetailsBatch(detailsToSave);
      }
      ...
      const printData = {
        ...
        wgtslipno: headerPayload.wgtslipno,
        loadslipno: headerPayload.loadslipno,
        details: this.detailsArray.controls.map((ctrl, i) => {
          const pktnoVal = ctrl.get('pktno')?.value;
          const pkt = this.pktmasterMap().get(pktnoVal);
          const fitno = pkt ? pkt['fitno'] : null;
          const mstfit = fitno ? this.mstfitMap().get(fitno) : null;
          const pcsperpkt = mstfit ? Number(mstfit['pcsperpkt']) || 0 : 0;
          return {
            srno: i + 1,
            pktname: this.getPktname(i),
            particulars: ctrl.get('particulars')?.value || this.getPktname(i),
  ...
  printInvoice(data: any, printWindow?: Window | null) {
    ...
    for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
      ...
      const pageDetails = details.slice(startIdx, endIdx);
      const isFirstPage = pageNum === 1;
      const isLastPage = pageNum === totalPages;

      const itemsHtml = pageDetails.map((d: any, idx: number) => {
        ...
        return `
          <tr>
            <td class="c">${startIdx + idx + 1}</td>
            <td class="r">${this.formatIndianNumber(rowAvg, 2)}</td>
            <td>${this.escapeHtml(d.particulars || '')}</td>
            <td></td>
      ...
      const pageTotalQty = pageDetails.reduce((sum: number, d: any) => sum + (Number(d.qty) || 0), 0);
      const pageTotalWeight = pageDetails.reduce((sum: number, d: any) => sum + (Number(d.weight) || 0), 0);
      const pageTotalAmount = pageDetails.reduce((sum: number, d: any) => sum + (Number(d.amount) || 0), 0);
      const pageAvgRate = pageTotalWeight > 0 ? pageTotalAmount / pageTotalWeight : 0;

      const minRows = isFirstPage ? 16 : 20;
      const blankRows = Math.max(0, minRows - pageDetails.length);
        ...
        <div class="sheet">
          <table class="title-grid">
            <tr>
              ...
              <td class="page">Page ${pageNum} of ${totalPages}</td>
            </tr>
          </table>
            ...
            <tr>
              ...
              <td style="width:38%" class="right-box">
                <table>
                  ...
                  <tr><td>Weight Slipno:</td><td>${this.escapeHtml(data.wgtslipno || '')}</td></tr>
                  <tr><td>Load Slipno:</td><td>${this.escapeHtml(data.loadslipno || '')}</td></tr>
                </table>
              </td>
            </tr>
          </table>
          ...
          <table class="totals-row">
            <tr>
              <td class="label">Avg Rate:</td>
              <td class="value">${this.formatIndianNumber(pageAvgRate, 2)}</td>
              <td class="label">Total Bandl</td>
              <td class="value"></td>
              <td class="label">Total  Pcs</td>
              <td class="value">${this.formatQty(pageTotalQty)}</td>
              <td class="label">Total  Kgs</td>
              <td class="value">${this.formatIndianNumber(pageTotalWeight, 2)}</td>
              <td class="value">${this.formatIndianNumber(pageTotalAmount, 2)}</td>
            </tr>
          </table>
          <table class="bottom-wrap">
            <tr>
              <td style="width:62%" class="left-bottom"></td>
              <td style="width:38%" class="right-bottom">
                <table>
                  <tr><td class="slabel">Page Total :</td><td class="sval">${this.formatIndianNumber(pageTotalAmount, 2)}</td></tr>
                </table>
              </td>
            </tr>
          </table>
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .sheet { width: 100%; border: 1px solid #000; margin-bottom: 20px; }
            ...
            .items th:nth-child(1), .items td:nth-child(1) { width: 4%; }
            .items th:nth-child(2), .items td:nth-child(2) { width: 7%; }
            .items th:nth-child(3), .items td:nth-child(3) { width: 37%; }
            ...
            .items th:nth-child(8), .items td:nth-child(8) { width: 14%; }
  ...
  async printInvoiceDirect(inv: any) {
    const printWindow = window.open('', '_blank');
    if (!printWindow) {
      this.toastService.error('Please allow popups to print');
      return;
    ...
    printWindow.document.write(`
      <html>
        <head><title>Loading Invoice...</title></head>
        <body style="font-family: Arial, sans-serif; padding: 24px;">Loading invoice...</body>
      </html>
    `);
    printWindow.document.close();
    ...
    try {
      ...
      const printData = {
        ...
        details: details.map((d: any, i: number) => {
          const pkt = this.pktmasterMap().get(d.pktno);
          const fitno = pkt ? pkt['fitno'] : null;
          const mstfit = fitno ? this.mstfitMap().get(fitno) : null;
          const pcsperpkt = mstfit ? Number(mstfit['pcsperpkt']) || 0 : 0;
          console.log('Row', i+1, 'pktno:', d.pktno, 'pkt:', pkt, 'fitno:', fitno, 'mstfit:', mstfit, 'pcsperpkt:', pcsperpkt);
          return {
            srno: i + 1,
            pktname: pkt ? pkt['pktname'] : d.pktno,
            particulars: d.particulars || pkt?.['pktname'] || '',
            qty: d.qty || 0,
            weight: d.weight || 0,
            rate: d.rate || 0,
            amount: d.amount || 0,
        ...
        totalwgt: inv.totalwgt || 0,
        billamt: inv.billamt || 0,
        other1_amt: inv.other1_amt || 0,
        other2_amt: inv.other2_amt || 0,
        loadamt: inv.loadamt || 0,
        gstper: inv.gstper || 0,
        gstamt: inv.gstamt || 0,
        nettamount: inv.nettamount || 0
      };
      ...
      this.toastService.error('Failed to load invoice for printing');
      printWindow.close();
    }
  }
