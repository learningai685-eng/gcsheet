...
import { GcsheetGradeService } from '../../services/gcsheet-grade.service';
import { ToastService } from '../../shared/toast/toast.service';
import { PaginationComponent } from '../../shared/pagination/pagination.component';

@Component({
  selector: 'app-gcsheet-grade',
  standalone: true,
          ...
          <ul class="navbar-nav me-auto">
            ...
            <li class="nav-item dropdown">
              ...
              <ul class="dropdown-menu" [class.show]="showGcsheetMenu">
                ...
                <li><a class="dropdown-item active" style="color: #0d6efd; font-weight: 500;"><i class="bi bi-bar-chart me-2"></i>Grade</a></li>
    ...
    <div class="container mt-5">
      <div class="card">
        <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
          <h4 class="mb-0"><i class="bi bi-bar-chart me-2"></i>Gcsheet Grade Master</h4>
          <div class="d-flex gap-2">
            ...
            <button class="btn btn-light btn-sm" (click)="openAddModal()"><i class="bi bi-plus-circle me-1"></i>Add Grade</button>
          </div>
        </div>
    ...
    @if (showModal()) {
      <div class="modal show d-block" tabindex="-1" style="background-color: rgba(0,0,0,0.5);">
        <div class="modal-dialog"><div class="modal-content">
          <div class="modal-header"><h5 class="modal-title">{{ isEditMode() ? 'Edit' : 'Add' }} Grade</h5><button type="button" class="btn-close" (click)="closeModal()"></button></div>
          <div class="modal-body">
            <form [formGroup]="form">
              <div class="mb-3"><label class="form-label">Name *</label><input type="text" class="form-control" formControlName="name" (blur)="checkDuplicateName()">
...
export class GcsheetGradeComponent implements OnDestroy {
  private service = inject(GcsheetGradeService);
  ...
  async saveData() {
    ...
    try {
      const result = this.isEditMode() 
        ? await this.service.update(this.currentItem().id, { name: this.form.value.name! }) 
        : await this.service.create({ name: this.form.value.name! });
      if (result.success) { 
        this.toastService.success(`Grade ${this.isEditMode() ? 'updated' : 'created'} successfully`); 
        this.closeModal(); 
        await this.loadData(); 
      } else { 
  ...
  exportToExcel(): void {
    ...
    setTimeout(() => { 
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Grade Master'); 
      XLSX.writeFile(wb, `gcsheet_grade_export_${new Date().toISOString().split('T')[0]}.xlsx`); 
